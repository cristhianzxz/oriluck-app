...
import { setBingoHousePercent } from "../firestoreService";
import {
  doc,
  onSnapshot,
  collection,
...
const OwnerPanel = () => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, (snap) => {
      if (snap.exists()) {
        const d = snap.data();
        setHouseFund(d.totalForHouse || 0);
  ...
  const handleSavePercent = async () => {
    try {
      await setBingoHousePercent(Number(newPercent));
      const q = query(
      ...
      snapshot.forEach(docSnap => {
        updates.push(
          updateDoc(doc(db, 'bingoTournaments', docSnap.id), {
            percentageHouse: Number(newPercent)
          })
        );
      });
  ...
  if (verifyingAccess) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-yellow-300 text-xl">
        Verificando acceso...
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-yellow-900 flex flex-col">
        ...
        <div className="w-full flex flex-col items-center justify-center py-16">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 w-full max-w-4xl mt-12">
            <button
              ...
              className="bg-yellow-500 hover:bg-yellow-400 text-black font-bold py-12 px-8 rounded-2xl shadow-xl text-3xl flex flex-col items-center justify-center transition-all"
            ...
            <button
              ...
              className="bg-blue-700 hover:bg-blue-600 text-white font-bold py-12 px-8 rounded-2xl shadow-xl text-3xl flex flex-col items-center justify-center transition-all"
              style={{ minHeight: "220px" }}
            >
            ...
            <button
              disabled
              className="bg-gray-700 text-white font-bold py-12 px-8 rounded-2xl shadow-xl text-3xl flex flex-col items-center justify-center opacity-70 cursor-not-allowed"
              style={{ minHeight: "220px" }}
            >
              🛠️ Función 3
          ...
          <button
            ...
            className="mt-16 bg-gray-700 hover:bg-gray-800 text-yellow-200 px-8 py-4 rounded-xl text-2xl shadow-lg"
          >
      ...
      {showBolsa && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-sm">
          <div className="bg-black/80 rounded-3xl p-8 shadow-2xl border border-yellow-500/30 max-w-lg w-full relative">
            <button
            ...
            {loading ? (
              ...
              <>
                <div className="mb-4 text-center">
                  ...
                  <div className="text-yellow-100 mb-2">Acumulado total por la casa (varios juegos)</div>
                </div>
                <div className="flex items-center justify-center gap-4 my-4">
                  <div className="font-semibold text-yellow-300">Porcentaje actual para la casa (Bingo):</div>
                  {editPercent ? (
                    <input
                      type="number"
                      min={1}
      ...
      {showMovements && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60 backdrop-blur-sm">
          <div className="bg-black/90 rounded-3xl p-8 shadow-2xl border border-blue-500/30 max-w-2xl w-full relative">
            <button
                      ...
                      <div key={mov.id} className="bg-blue-900/20 rounded-xl p-4 border border-blue-500/10 flex flex-col sm:flex-row sm:items-center gap-2 justify-between">
                        ...
                        <div>
                          <div className="font-bold text-yellow-300">
                            {mov.action === "add" ? "+" : mov.action === "subtract" ? "-" : ""}{mov.amount?.toLocaleString()} Bs
                          </div>
