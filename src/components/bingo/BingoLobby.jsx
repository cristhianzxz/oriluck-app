...
const BingoLobby = () => {
    ...
    // --- NUEVOS ESTADOS Y LÓGICA DE PERFIL ---
    const [currentUserData, setCurrentUserData] = useState(undefined);

    useEffect(() => {
        // Si no hay usuario autenticado (null), establecemos currentUserData a null.
        if (!currentUser?.uid) {
            setCurrentUserData(null);
            setLoading(false); // No hay datos de Firebase que cargar
            return;
        }
        
        // Listener para cargar el perfil (role, balance, phone, etc.)
        const userRef = doc(db, "users", currentUser.uid);
        const unsub = onSnapshot(userRef, (snap) => {
            if (snap.exists()) {
                setCurrentUserData(snap.data());
                // El balance se cargará aquí, no en un listener separado
                setUserBalance(snap.data().balance || 0); 
            } else {
                // Si el documento no existe (raro), asumimos un rol por defecto.
                setCurrentUserData({ role: "user" });
                setUserBalance(0);
            }
        });

        // Retornamos el unsubscribe de Firebase y no necesitamos un listener de balance separado.
        return () => unsub();
    }, [currentUser?.uid]);
    ...
    // 1. useEffect de Redirección: Maneja el estado de autenticación
    useEffect(() => {
        // Redirige si el estado de autenticación (currentUser) se resolvió a null (no logeado)
        if (currentUser === null) {
            navigate('/lobby');
            return;
        }
    ...
    // 2. useEffect principal: Carga inicial de datos de torneos (depende de la autenticación)
    useEffect(() => {
        // Espera a que el usuario esté autenticado y su data (incluyendo el rol) se haya cargado
        ...
        const unsubActive = onSnapshot(activeQuery, snapshot => {
            ...
            // Marcamos como cargado después de obtener los torneos
            setLoading(false); 
        ...
        // Solo retornamos el unsubscribe de los torneos, el del usuario está arriba.
        return () => {
            unsubActive();
            unsubFinished();
        };
    ...
    // 3. NUEVO useEffect: Listener para el torneo seleccionado (detalles).
    useEffect(() => {
    ...
    // ------------------------------------------------------------------
    // Prioridad 1: Cargando estado de autenticación de Firebase (AuthContext)
    if (currentUser === undefined) {
        return <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">Cargando credenciales de sesión...</div>;
    }
    
    // Prioridad 2: Cargando el perfil del usuario (role, balance)
    if (currentUserData === undefined) {
        return <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">Cargando perfil de usuario...</div>;
    }

    // Prioridad 3: Cargando datos iniciales de torneos de Firebase
    if (loading) {
        return <div className="min-h-screen flex items-center justify-center bg-gray-900 text-white">Cargando torneos...</div>;
    }
    // ------------------------------------------------------------------
