...
const ChatPanel = () => {
    ...
    const handleSend = () => {
        if (input.trim()) {
            setMessages(prev => [...prev, { user: 'Yo', text: input, isUser: true, id: Date.now() }]);
            setInput('');
        }
    };
    ...
    return (
        <div className="panel p-4 flex flex-col flex-grow min-h-0">
            ...
            <div id="chat-messages" className="flex-grow overflow-y-auto pr-2 space-y-3 text-sm">
                {messages.map((msg) => (
                    <div key={msg.id}><span className={`font-bold ${msg.isUser ? 'text-cyan-400' : 'text-purple-400'}`}>{msg.user}: </span><span>{msg.text}</span></div>
                ))}
                <div ref={chatEndRef} />
            </div>
...
const GameScreen = ({ gameState, multiplier, countdown, rocketPosition, gridOffset, onConfettiTrigger }) => {
    ...
    return (
        <div className="game-screen flex flex-col items-center justify-center">
            ...
            {showRoundElements && <div id="multiplier-display" className={`multiplier ${gameState === 'crashed' ? 'crashed' : ''}`}>{multiplier.toFixed(2)}x</div>}
...
const BetPanel = ({ onBet, onCancel, onCashout, gameState, currentBet, multiplier }) => {
    ...
    useEffect(() => {
        if (isAutoBet && gameState === 'waiting' && !currentBet) {
            onBet(parseFloat(betAmount), isAutoCashout ? parseFloat(autoCashoutAmount) : 0);
        }
    }, [isAutoBet, gameState, currentBet, onBet, betAmount, isAutoCashout, autoCashoutAmount]);

    const handleActionClick = () => {
        const state = getButtonState();
        if (state.text.startsWith("RETIRAR")) onCashout();
        else if (state.text === "CANCELAR APUESTA") onCancel();
        else onBet(parseFloat(betAmount), isAutoCashout ? parseFloat(autoCashoutAmount) : 0);
    };
    
    const getButtonState = useCallback(() => {
        if (gameState === 'waiting' && currentBet) return { text: "CANCELAR APUESTA", className: "btn-cancel", disabled: false };
        if (gameState === 'running') {
            if (currentBet) {
                if (currentBet.status === 'cashed_out') return { text: `GANASTE ${currentBet.winnings.toFixed(2)} VES`, className: "btn-success", disabled: true };
                return { text: `RETIRAR ${(currentBet.bet * multiplier).toFixed(2)} VES`, className: "btn-cashout", disabled: false };
            }
        }
    ...
    const isBetPlaced = !!currentBet;

    return (
        <div className="bet-panel panel p-4 flex flex-col space-y-3">
            <div className="flex items-center gap-4">
                <div className="flex flex-col items-center pt-5"><label className="text-sm font-medium text-gray-400">Auto</label><div onClick={() => setIsAutoBet(!isAutoBet)} className={`toggle-switch mt-1 ${isAutoBet ? 'active' : ''}`}><div className="toggle-switch-slider"></div></div></div>
            ...
            <div className="flex items-center gap-4">
                <div className="flex flex-col items-center pt-5"><label className="text-sm font-medium text-gray-400">Auto</label><div onClick={() => setIsAutoCashout(!isAutoCashout)} className={`toggle-switch mt-1 ${isAutoCashout ? 'active' : ''}`}><div className="toggle-switch-slider"></div></div></div>
...
const RightColumn = ({ myBets, activeBets, gameState }) => {
    ...
    return (
        <div className="right-column panel p-4 flex flex-col"><div className="flex border-b border-gray-700 mb-2"><button onClick={() => setActiveTab('active')} className={`tab-button flex-1 py-2 font-semibold ${activeTab === 'active' ? 'active' : ''}`}>Apuestas Activas</button><button onClick={() => setActiveTab('my')} className={`tab-button flex-1 py-2 font-semibold ${activeTab === 'my' ? 'active' : ''}`}>Mis Apuestas</button></div>
            ...
            <div className={`flex-grow overflow-y-auto text-sm space-y-2 pr-2 ${activeTab !== 'my' ? 'hidden' : ''}`}>
                {myBets.map((bet) => { 
                    ...
                    const profitLoss = isWin ? bet.winnings - bet.bet : -bet.bet;
                    return ( 
                        <div key={bet.id} className="grid grid-cols-3 gap-2 items-center bg-gray-900/50 p-2 rounded-md">
                            <span className={`font-semibold ${isWin ? 'text-green-400' : 'text-red-400'}`}>{isWin ? `+ ${profitLoss.toFixed(2)}` : `${profitLoss.toFixed(2)}`} VES</span>
...
const RocketCrashGame = () => {
    ...
    useEffect(() => {
        ...
        // Asumiendo que las apuestas históricas se guardan en `crash_bets_history`
        const q = query(collection(db, "crash_bets_history"), where("userId", "==", currentUser.uid), orderBy("timestamp", "desc"), limit(20));
        const unsub = onSnapshot(q, (snapshot) => {
            const betsData = snapshot.docs.map(doc => {
                const data = doc.data();
                return {
                    id: doc.id,
                    amount: data.bet, status: data.status,
    ...
    useEffect(() => {
        const unsub = onSnapshot(doc(db, 'game_crash', 'live_game'), (snap) => {
            if (snap.exists()) {
                ...
                // Listener para apuestas activas
    ...
    useEffect(() => {
        const cleanup = () => { clearInterval(intervalsRef.current.countdown); clearInterval(intervalsRef.current.particles); cancelAnimationFrame(animationFrameRef.current); };
        ...
        if (gameStateData.gameState === 'waiting') {
            cleanup();
            setMultiplier(1.00); setRocketPosition({ x: 10, y: 30 }); setGridOffset(0);
        ...
        } else if (gameStateData.gameState === 'running') {
            cleanup();
            const startTime = gameStateData.started_at.toMillis();
            const k = gameStateData.rocketPathK;
            
            intervalsRef.current.particles = setInterval(() => {
                const rocketEl = document.getElementById('rocket'); if (!rocketEl) return;
                const particle = document.createElement('div'); particle.className = 'particle';
                document.getElementById('particle-container')?.appendChild(particle);
                particle.style.left = `${rocketEl.offsetLeft + rocketEl.offsetWidth * 0.2}px`;
                particle.style.top = `${rocketEl.offsetTop + rocketEl.offsetHeight * 0.85}px`;
            ...
            const animate = () => {
                ...
                if (gameScreen) {
                    ...
                    setRocketPosition({ x: 10 + Math.min(elapsed / 50, 1) * 65, y: (gameScreen.clientHeight * 0.05) + Math.min(totalTravelY, scrollThreshold) });
                    setGridOffset(totalTravelY > scrollThreshold ? totalTravelY - scrollThreshold : 0);
                }
                
                if (autoCashoutTarget > 0 && currentMultiplier >= autoCashoutTarget && currentBet && currentBet.status === 'playing') handleCashout();
                
                animationFrameRef.current = requestAnimationFrame(animate);
            };
            animationFrameRef.current = requestAnimationFrame(animate);
        } else if (gameStateData.gameState === 'crashed') {
            cleanup();
            setMultiplier(gameStateData.crashPoint);
        }
        return cleanup;
    }, [gameStateData, autoCashoutTarget, currentBet]);
    
    const handleBet = async (amount, autoCashout) => {
        if (!currentUser || balance < amount || amount < 3 || gameState !== 'waiting') {
            addToast('No se puede apostar en este momento.', 'error');
            return;
        }
        try {
            const placeBetFunc = httpsCallable(functions, 'placeBet_crash');
            await placeBetFunc({ amount });
            setAutoCashoutTarget(autoCashout || 0);
        } catch (error) {
    ...
    const handleCancelBet = async () => {
        if (currentBet && gameState === 'waiting') {
            try {
                // El backend debería tener una función para esto.
                // Como fallback, se puede eliminar el doc, pero es menos seguro.
                console.log("Llamar a la función de cancelar apuesta (si existe)");
                addToast('Función de cancelar no implementada en backend.', 'info');
            } catch (error) {
            ...
            <main className="flex-grow min-h-0">
                <div className="main-grid">
                    ...
                    <div className="center-column">
                        ...
                        <BetPanel onBet={handleBet} onCancel={handleCancelBet} onCashout={handleCashout} gameState={gameState} currentBet={currentBet} betForNextRound={null} hasCashedOut={currentBet?.status === 'cashed_out'} lastWinnings={currentBet?.winnings || 0} multiplier={multiplier} />
                    </div>
